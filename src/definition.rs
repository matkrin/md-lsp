...
use crate::{
    ast::{
        find_definition_for_identifier, find_foot_definition_for_identifier, find_link_for_position,
    },
...
pub fn definition(params: &GotoDefinitionParams, state: &State) -> Option<GotoDefinitionResponse> {
    ...
    location.map(|loc| GotoDefinitionResponse::Scalar(loc))
}

pub fn def_handle_link_to_heading(req_uri: &Url, link: &Link, state: &State) -> Option<Location> {
    match resolve_link(link, state) {
        ResolvedLink::File { link, file_uri } => Some(Location {
            uri: file_uri.clone(),
            range: range_zero(),
        }),
